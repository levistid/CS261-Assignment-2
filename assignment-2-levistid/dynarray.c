 ...
 * documentation below for more information on the indiidual functions in
 * this implementation.
 */
...
void* top_of_dynarry(struct dynarray* val){
  return val->data[val->start];
}

void add(struct dynarray* da, void* val){
  int i;
  if(da->size >= da->capacity){
    _dynarray_resize(da, 2 * da->capacity);
  }
  i = (da->size + da->start) % da->capacity;
  da->data[i] = val;
  da->size++;
}

void delete_val(struct dynarray* val){
  int i;
  i = (val->start + 1) % val->capacity;
